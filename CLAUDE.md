...
**CLI subcommands** (all in `src/main.rs`): `serve`, `ingest`, `import`, `watch`, `identify` (`--doi`/`--arxiv`/`--title`), `refresh`, `search`, `export` (BibTeX/BibLaTeX), `project`, `tag`, `star`, `unstar`, `index` (`status`/`rebuild`), `summarize`, `delete`/`restore`/`purge`, `proxy-cookie`.
